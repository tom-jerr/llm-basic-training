...
from src.position_encoding import RoPE  # 假设你有对应的 PyTorch RoPE 实现
...
def apply_rotary_emb(
    ...
    # xq_ 与freqs_cis广播哈达玛积, 两个复数的乘法实现
...
def apply_rotary_emb_qwen2(
    ...
    # Qwen2 style RoPE: 头嵌入维度分为两半，分别应用不同的频率
    # xq.shape = [bsz, seqlen, self.n_local_heads, self.head_dim]
...
def rope_helper(
    ...
    for _ in range(100):
        user_layer = RoPE(HEAD_DIM, MAX_SEQ_LEN, BASE, traditional=traditional).to(
            device
        )
        x = torch.rand(
...
def test_task_1_rope_torch_traditional(device, with_offset, precision):
    rope_helper(device, True, precision, with_offset)
...
def test_task_2_rope_torch_non_traditional(device, with_offset, precision):
    rope_helper(device, False, precision, with_offset)
