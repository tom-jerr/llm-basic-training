...
void cublas_gemm(float *A, float *B, float *C, size_t M, size_t N, size_t K) {
...
void cublas_gemm_tf32(float *A, float *B, float *C, size_t M, size_t N, size_t K) {
  cublasHandle_t handle = nullptr;
  cublasCreate(&handle);
...
void gemm_cublas(torch::Tensor a, torch::Tensor b, torch::Tensor c) {
  ...
  cublas_gemm(reinterpret_cast<float *>(a.data_ptr()), reinterpret_cast<float *>(b.data_ptr()),
...
void gemm_cublas_tf32(torch::Tensor a, torch::Tensor b, torch::Tensor c) {
  ...
  cublas_gemm_tf32(reinterpret_cast<float *>(a.data_ptr()), reinterpret_cast<float *>(b.data_ptr()),
                   reinterpret_cast<float *>(c.data_ptr()), M, N, K);
}